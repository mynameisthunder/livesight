...
extension ViewController: MessagePresenting {
    ...
    // These put the nodes along the line in AR need to be set on the floor plane which is not hard
    // AR has something for that
    ...
    private func addSphere(for step: MKRouteStep) {
        ...
        let nodeColour: UIColor = UIColor(red:CGFloat(4.0/255.0), green:CGFloat(120.0/255.0), blue:CGFloat(225.0/255.0), alpha:CGFloat(0.78))
        sphere.addNode(with: 0.8, and: nodeColour, and: step.instructions)
    ...
    // These put the nodes along the lines on the map - change this b/c it's really uncalled for
